...
const actions = {
  ...
  filterRoutes(context, menus) {
    console.log('用户可以访问的权限',menus)
    console.log('所有动态路由',asyncRoutes)
    const routes = [];
    // 筛选出 动态路由和menus中能够对上的路由
    menus.forEach((key) => {
      // asyncRoutes 找 有没有对象中的path属性等于key的 如果找不到就没有权限 如果找到了 就要筛选出来
      routes.push(...asyncRoutes.filter((item) => item.path.includes(key))); // 得到一个数组 有可能 有元素 也有可能是空数组
    });
